...
echo "export PATH=$FIELDDB_HOME/node/bin:$PATH" >> ~/.bashrc
source ~/.bashrc
...
./configure --prefix=$FIELDDB_HOME/couchdb
make
make install

# Put couchdb on the path so you can run it by using just couchdb
echo "export PATH=$FIELDDB_HOME/couchdb/bin:$PATH" >> ~/.bashrc
source ~/.bashrc
# Turn on couchdb
couchdb & 
...
node $FIELDDB_HOME/FieldDBWebServer/server.js &
...
node $FIELDDB_HOME/AudioWebService/service.js &

echo "Testing if FieldDB LexiconWebService will run, it should say 'Listening on 3185' "
bash  $FIELDDB_HOME/LexiconWebService/service.sh &
...
then
  # do dangerous stuff
	...
	sudo cp $FIELDDB_HOME/AuthenticationWebService/nginx_sample.conf /etc/nginx/conf.d/default.conf 
	sudo cp $FIELDDB_HOME/AuthenticationWebService/nginx_sample_proxy.conf /etc/nginx/proxy.conf 
