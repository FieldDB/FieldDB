...
// Implements the PouchDB API for dealing with CouchDB instances over HTTP
var HttpsPouch = function(opts, callback) {
  ...
  // The functions that will be publically available for HttpsPouch
  var api = {};
  ...
  // The HttpxPouch's ID is its URL
  api.id = function() {
    return genUrl(host, '');
  };
  ...
  // Replicate from the database given by url to this HttpsPouch
  api.replicate.from = function(url, opts, callback) {
    // If no options were given, set the callback to be the second parameter
    if (opts instanceof Function) {
      callback = opts;
  ...
  // Replicate to the database given by dbName from this HttpsPouch
  api.replicate.to = function(dbName, opts, callback) {
    // If no options were given, set the callback to be the second parameter
    if (opts instanceof Function) {
      callback = opts;
...
// Delete the HttpsPouch specified by the given name.
HttpsPouch.destroy = function(name, callback) {
  var host = getHost(name);
  ajax({auth: host.auth, type: 'DELETE', url: genUrl(host, '')}, callback);
};

// HttpsPouch is a valid adapter.
HttpsPouch.valid = function() {
  return true;
}

// Set HttpsPouch to be the adapter used with the https scheme.
Pouch.adapter('https', HttpsPouch);
