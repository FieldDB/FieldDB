...
\section {Statement of Need/Context}

The FieldLinguists' App is conceived out of the needs of language researchers doing field work. Linguistic field work often requires researchers to travel to places where a stable connection to the internet is not guaranteed. Also, it often involves a group of researchers contributing to building a single database.
...
There are existing programmes/software used for linguistic field work, 
however, they fall short in providing features necessary for collaborative field work. 
For example, some web-based databases (e.g.  {\it Karuk Dictionary and Texts} http://linguistics.berkeley.edu/{\textasciitilde}ka
ruk/links.php,   {\it The Washo Project} http://washo.uchicago.edu/dictionary/dictionary.php) work only online, hence it is impossible for researchers to enter new data or search the database while on the field where the internet is unavailable.  
...
are excellent in terms of annotating data and building data into various types (corpus, grammar or lexicon). Nonetheless, integration of data taken by individual researchers is not well-automated and takes considerable work of researchers. Moreover, they have a particular preference of platforms (either PC or Linux, but not both), which causes further difficulty in data sharing among researchers using different platforms.  
...
\item Both on- and off-line functionality so that the field work is not constrained by the internet accessibility. 
