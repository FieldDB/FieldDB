...
This application will be able to perform the necessary functions needed by fieldlinguists. The dashboard will be composed of several widgets. The Data Entry widget will be the primary focus, containing three core fields customary for a gloss format (utterance, gloss, translation). In addition to these, researchers will be able to add customized fields, such as phonetic transcription or context for an utterance. Furthermore, researchers can add tags for categorization and mark the status of each individual data entry as "Checked" or "To be checked with consultant", which further aids organization and reduces the number of errors that inevitably occur during research. Other functions such as importing data and exporting data into various formats aid efficiency and convenience. 

Another widget will be an Activity Feed View displaying the most recent changes. These may include comments made on data entries, recent authorings, and recent edits. For teams in particular, this will help keep track of who made which changes and when. 

Two more widgets will be a Corpus Information widget and a Data List widget. The Corpus Information widget will show details about the current session and will display a menu of things the user can do (ex. open a new session, browse all entries, etc.).  The Data List widget will show a list of data when a researcher runs a search. This list can be further filtered---researchers can "star" favorites and add them to a more specific list. Lists may be categorized, which makes the data easily accessible. 
