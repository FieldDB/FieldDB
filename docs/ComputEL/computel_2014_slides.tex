...
%\setbeameroption{show notes on second screen} % My version of beamer must be too old for this version...
\usetheme{PaloAlto}
...
\begin{frame}
\titlepage
\note{Hello everyone. My name is Joel Dunham and this is Gina}
...
\note[item]{So, it hardly needs to be stated that we are in a situation where
    ...
    or "sleeping" to use the jargon of more optimistic commentators, in the
    very near future.}
\note[item]{Linguistic diversity and language vitality are valuable from a
    number of perspectives. For those who might be sceptical of the value of
    language vitality to social wellbeing, the Hallett et al. (2007) reference
    here is to a study which finds a correlation between higher levels of
    Aboriginal language use in British Columbia First Nations bands and lower
    youth suicide rates.}
\note[item]{And clearly we're all here because we think that computational
    ...
    documentation, study, and revitalization of these endangered languages.}
...
\begin{itemize}
    \item<1-> Documentation, description, and/or theoretical analysis?
        ...
        \begin{itemize}
            ...
            \item<3->Better description can prevent data graveyards (Beale
                2014, Bender et al. 2014)
            ...
            \item<4->Theoretical linguistic analysis uncovers previously
                ...
                \note[item]<4>{There is a related debate over the role of 
                    ...
                    responding in defense of a hypothesis-driven approach and the, to some,
                    controversial elicitation methods, e.g., grammaticality judgments
                    translation requests, that it entails. Sarah Murray, in a
                    forthcoming paper, argues that the theoretical linguistic approach
                    can lead to the discovery of significant generalizations that would
    ...
    \item<5->A duty to collaborate?
        ...
        \end{itemize}
    \item<8->Whatever we decide, let's maximize openness, access, sharing (data
        and functionality)
...
\begin{tikzpicture}[
	  ...
	  % Position the nodes using a matrix layout
	  \matrix{
	    \node[researcher] (researcher1) {field\\researcher};  \&  \& \node[researcher] (researcher2) {researcher};\\
...
\note[item]<1>{We are not claiming that LingSync and the OLD fully meet
    all of these requirements. These are goals we are working towards.}
...
\note[item]<2>{The software must be able to handle primary data. In particular,
    ...
    stimuli). The system should allow for the alignment of audio/video with
    transcriptions and annotations and textual and audio/video data should be
    displayed simultaneously. It should be possible to record audio right into
    the application and, if possible, the text/audio alignment process should
...
\end{requirement}
\end{uncoverenv}
\note[item]<3>{The system should facilitate the curation of data. For example,
    initial elicitation can involve simply the uploading of recordings. Then,
    subsequent waves of data curation can involve transcription at various
    levels or the creation of morphological analyses and annotations of various
    types, for example, tagging and categorizing.}
...
\note[item]<6>{The system should contribute to user productivity. It should
    include well-designed interfaces as well as conveniences which expedite
...
\end{center}
\end{figure}
\note{And many user interfaces for different stakeholders in mobile and desktop contexts}
\end{frame}
...
\end{center}
\end{figure}
\note{letting the corpus grow for many purposes}
\end{frame}
...
\end{itemize}
\note{Now I'd like to discuss the OLD's morphological parser functionality. I
    will present and motivate the OLD's approach to creating morphological
    parsers with reference to two parsers created for the Blackfoot language.}
\end{frame}
...
\note{Here are the requirements that guided the design of the OLD's
        ...
        \begin{itemize}
            ...
            \item<3->make use of data in the system
                \note[item]<3>{Fieldworkers should be able to build parsers
                    ...
                    IGT format and lexical entries.}
...
\note[item]{The morphophonology is a finite-state transducer that takes a
    ...
    output; it is the composition of a phonology FST and a morphology FST.}
...
\begin{itemize}
    \item<1->Phonology: a transducer wholly defined by the fieldworker as ordered
        ...
        \note[item]<1>{An OLD phonology is a transducer that is wholly defined by
            the fieldworker as an ordered list of context-sensitive rewrite
            rules} 
\end{itemize}
...
\note[item]<3>{The phonology is written in the regular expression rewrite rule
    language that is accepted by FST tools like foma and Xerox FST.}
...
\begin{itemize}
    ...
    \item<5->facilitate automated analysis testing
        \note[item]<5>{Also, since the fieldworker has full control over the
            ...
            component of the grammar. For example, the words in the database that
            the morphophonology cannot recognize may lead to modifications of
...
\end{tabular}\xe
\end{uncoverenv}
\note[item]<2>{(\getref{mtactic}) is a rewrite rule representation
of the morphological generalizations that can be extracted from a corpus
of forms like (\getref{danced}) and implemented as an FST, in either the lexc
or regex language.}
...
\note{We would like to thank our linguistics student interns, computer science student interns and countless other open source software developers who directly or indirectly helped build LingSync/OLD to what it is today and will be in the future. \\
~\\
