...
\subsubsection{Users expectations}
...
LingSync uses a map reduce function which indexes data in a users corpus and generates precedence rules which are used to seed finite state automata  \cite{Cook:2009} which represent current morphological templates in the database. In this way the glosser is ``trained" on the user's existing segmentation and glossing and automatically ``learns" as the user adds more data and the glossing/segmentation evolves over the course of data collection and analysis. While the implementation of the glosser is arguably simple in its approach, it has nonetheless received favorable comments from users who are used to existing semi-automatic glossing systems. The glosser's approach is different from FLEx which uses explicit morphological subcategorization or rules  \cite{Black:2006} which the user can define and later use in their description of the language.

LingSync has a lexicon browser component which permits users to browse the ``learned"  relations between morphemes, and clean the data for consistency.  To satisfy the researchers desire to document generalizations which might not be immediately evident in the primary data  users are permitted to add declarative knowledge to learned rules but LingSync does not permit explicit addition of rules which are not gleaned from the data. While this is arguably a similar functionality to  curated lexicon or collocations dictionary, it encourages more grounding in primary data for language learners as all rules must have at least one example context in the corpus.
