...
%The existing linguistic database programs, although useful, have various shortfalls that would hinder collection and integration of data. Some of them are constrained by the internet accessibility or by the computer platform types. Some others demand extra human work in order to integrate data. Data entry can require hours of a researcherÕs time. 
...
There is an Android Elcitiation app which permits recording of high quality video on 7 inch or 10inch tablets during elicitaiton sessions. Multiple videos can be assocaited to a session and uploaded to the ProsodyLab Aligner webservice for automatic generation of aligned TextGrids. As opportunities allow, the above are only a few of the plugins which are reasonably seamless to integrate into LingSync. There are other plugins which can help use advances in computational linguistics  and natural language processing libraries  \citep{Chen:2011}  \citep{Rosenberg:2012} \citep{Sateli:2013}.%\citep{Desilets:2010}
