...
\papertitle{iField: A Free Tool for Creating and Maintaining a Shared Database For Communities, Linguists and Language Learners}

\firsttitle{\LARGE iField: \\[.2ex]\Large   A Free Tool for Creating and Maintaining a Shared Database For Communities, Linguists and Language Learners}
\runningtitle{iField}
...
Android Montr\'eal and Javascript Montr\'eal for their comments and feedback on aspects of iField presented at meet-ups,
...
Rakshit Majithiya,
Kim Dan Nguyen,
Pablo Duboue, as well as countless other open source software developers whose code directly or indirectly helped build iField to what it is today and in the future. 
We would like to thank iField users for providing feedback, suggestions,  asking questions and sending bug reports, all of which have been instrumental in iField's success.
We would like to thank the granting agencies which supported the students who have doubled as field work research assistants and interns on the project. 
...
iField was designed from the ground up to conform to E-MELD and DataOne data management best practices,
...
iField is hosted on cloud servers so that users can use it without knowing how to set up their own servers,
but it also has an installation guide for server administrators so organizations can run their own instance of iField. Not only is its source code 100\% open, iField's development is also 100\% open and driven by the research assistants of fieldwork teams who use the application.
\end{abstract}


\section{Introduction}

iField was conceived out of the needs of language researchers doing
...
these language researches often do not work with a database on a full-time basis but rather as one of many other tasks (including, but not limited to, teaching, grading, researching and preparing publications).
...
In this paper we hope to demonstrate how the iField project embodies Thieberger's call to action. Section \ref{sec:background} provides some background to the project including why \S \ref{sec:why} iField was created despite numerous existing solutions, and the data management best practices \S \ref{sec:design} underlying iField. 
Section \ref{sec:what} discusses iField's core \S \ref{sec:core} functionality as well as how iField empowers teams  with a plugin architecture \S  \ref{sec:plugins} enabling the  repurposing of existing resources and software libraries. Section \ref{sec:use} discusses how teams are currently using iField, and celebrates iField's  growing community of over 300 users in just 1.5 years since the project was launched at CAML.
...
While the rest of this paper (with the exception of Plugins \S \ref{sec:plugins}) targets a field linguist audience, a discussion of iField is incomplete without some discussion of why iField is unique among field databases.

iField is actually only an open ended  data schema (as shown in \ref{ex:intelligible}) % (Appendix \S \ref{appendix:datumjson}), 
...
%CouchDB's map reduce functions are a powerhouse for searching highly structured diverse data without pushing round data into square holes. iField add sample `bots' to CouchDB so teams can clean and restructure data to follow and update to new conventions and new analyses of the data.} 
...
c. & open data enabled,  &yet respect the wishes  & 2012 & CORS \\
...
In addition to the user-driven objectives in (\ref{ex:objectives}), iField was designed to comply with the recommendations from %in two respects: Data Documentation and Data Management. Data documentation will be evaluated following 
...
E-MELD describes seven common problems in digital language documentation identified in \cite{Bird:2003} and recommends solutions. We list below these seven problems and how iField implements these best practices.
...
iField allows users to design their annotation conventions per corpus and makes these conventions available as a help text next to each field of each record of the corpus, ensuring that conventions are never far away, and reducing tension among team members.
Although a consistent  terminology is necessary for cross-corpora integration  \citep{Schalley:2012}, since each team of users works within a slightly different linguistic framework or set of conventions, no pre-determined terminology is sufficiently flexible to meet the needs of all users. iField therefore permits researchers to use open-ended categories for annotation, but 
...
iField is built using web technologies which permit the application to be 100\% Unicode, including support for right-to-left orthographies as well as IPA symbols. iField data is stored as plain text, and exportable in human-readable formats including JSON, LaTeX, CSV, and XML, all of which are non-proprietary and compatible with applications commonly used in linguistic data collection and documentation. iField runs on Windows, Linux and Mac OS X, as well as on newer platforms such as Android, ChromeBook and iPad.
...
Inside iField, data is searchable via keywords, or within specific fields, using either simple strings or regular expressions (\ref{ex:searchable}). 
...
Much linguistic data sits in researchers' offices in the form of notebooks or tapes, or as files on their computers.  iField data is online, and team members can even replicate the entire database (\ref{ex:accessible}) onto their laptops or department servers. Unlike tape data and note cards, the data can be in many physical locations at one time, and even available to collaborators across the world. 
...
In iField, each record  and media resource of each corpus has a unique URL (\ref{ex:discoverable}a) which can be used for citation. 
...
Preservation of digital data is always confronted with the possibility that the data file format over time becomes obsolete and unsupported by new technology.\footnote{http://emeld.org/school/classroom/archives/finding-archives.html} iField stores data in a plain text JSON format  (\ref{ex:intelligible}). 
...
%With iField, researchers and language consultants (authors of data) have control over who can see, edit and/or export which part of their data. 
In iField, data and information designated as confidential are encrypted prior to storage in the database using the US Federal approved AES encryption standard, and are presented as masked data (\ref{ex:discoverable}b). 
...
\section{Features of iField}
\label{sec:what}

iField integrates the common features of existing fieldwork database software,
while addressing many of the data management practices of E-MELD and DataOne.\footnote{See the iField WhitePaper for a more extensive discussion which addresses DataOne best practices\\ https://github.com/OpenSourceFieldlinguistics/FieldDB/blob/master/docs/WhitePaper.pdf?raw=true}
iField's functionality can be divided into two groups: functionality for linguistic field databases,
and functionality for user friendly community driven software.
In this paper we will place more emphasis on the linguistic field database functionality and only gloss over some of the software engineering practices which make iField a usable and maintainable system. 
...
Data entry in iField goes beyond just typing or transcribing data.
While simply typing in or importing data is the most common use case,
iField also provides the ability to add comments to any data in the system.
...
iField makes use of  typeahead and autocomplete, 
...
iField allows users to create bots which partly automate these tasks.
...
One of iField's founding principles is that you should only need to enter data once.
Whether you enter it in an Excel Spreadsheet,
in a handout,
in ELAN or in FLEx,
you should be able to import it into iField without needing to re-enter the data.
 Each record in a iField database can have an unlimited number of fields,
...
which means that if you have an existing glosser, you can plug it in to iField.
...
which is loosely modelled after a mental lexicon,\footnote{It is important that note that iField uses a lexicon, not a dictionary. The lexicon is a connected graph similar to theoretical models of mental lexicons, and is useful for linguistic analysis but not useful for producing dictionaries for language communities. iField is not suitable for curating dictionaries and the tasks involved in professional lexicography. Instead iField offers export to WeSay which makes it possible to build a community driven living dictionary. http://www.sil.org/resources/software\_fonts/wesay} 
...
iField was designed for powerful search.
...
For those users who like to think in Set Theory, iField provides the ability to look at the intersection or the union of search results. 
For phonologists, iField allows search using regular expressions to find segments in context.
...
Corpora in iField can be shared as a team,
with administrators,
...
but for larger corpora or where there is more activity, iField provides team activity feeds.
...
\cite{Thieberger:2012}  recommends that systems provide unique URLs so that researchers are able to cite primary resources, which further permits ``combining data from disparate sources which could be `mashups' or could, for example, involve correlating transcripts and media in `compound objects'.'' iField addresses both concerns by a fine grained level of sharing where the user controls what is shared and how, while each resource keeps a unique URL.\footnote{ \cite{Musgrave:2012} discusses the diverse ways a language community can benefit from language documentation and how different communities benefit differently. iField's sharing system permits prototyping `mashups' early in a collaboration that can result in data collection which suits not only language documentation efforts but also the needs of  unique language communities.} 
...
%Like EOPAS \citep{EOPAS:2014:Online} iField encourages all teams to adopt a Creative Commons license for their resources,
%however, each iField corpus team can choose to have its own terms of use (\ref{ex:customtermsOfUse}) and licensing which permits the maximum flexibility to teams.
...
%iField provides a high-contrast option as well as a dark option to reduce eyestrain after hours of entering data.
%iField was designed partly by research assistants who had entered data for 40 hours a week,
...
%Customization is a pervasive aspect of iField and has affected the way each functionality was conceived and implemented.


\subsubsection{Export}

As users of many diverse data management software,
we felt it was crucial that iField be non-proprietary and open. In a language documentation project,  linguistic data must remain usable even when ``the delivery system (which could be proprietary software or websites that are no longer maintained) becomes unusable'' \citep[p.132]{Thieberger:2012}.
...
iField is also able to export word lists,
...
\subsection{Plugging into iField}
\label{sec:plugins}


One of the strengths of iField is that it is built using well-understood web technologies which permit the creation and integration of nearly any existing software as web services. 
Even complex user interfaces can be combined and integrated with iField via the NPM and Bower web module management system.\footnote{
For teams collaborating with Computer Science or Software Engineering departments, there are limitless plugins which port advances in Computational Linguistics  \citep{Chen:2011}  or Natural Language Processing (NLP) including  negation and modality scope taggers  \citep{Rosenberg:2012}, %  using , 
discourse extraction out of social media  \citep{Dubuc:2010},  GATE \citep{Cunningham:2011} NLP web services for mobile apps \citep{Sateli:2013},
...
Sample web services which can wrap Bash/Python/Perl scripts  into web services are provided on the OpenSourceFieldlinguistics GitHub.\footnote{https://github.com/OpenSourceFieldlinguistics} %\citep{Desilets:2010}
}
...
\subsection{Users of iField so far}
\label{sec:use}

We have been very pleased by the positive reception so far towards iField by field linguists from a variety of locations and universites, and we gratefully acknowledge the comments and questions from all of our users which has resulted in substantial improvements in the system.
As of February 2014, only 1.5 years after iField was launched, there are over 300 iField users,
...
We estimate between 10-20 teams (consisting of 1-20 users each) have moved beyond the investigation phase, and have been using iField actively.

iField has also been used by five field methods classes so far, representing a typologically diverse range of languages.  In Winter 2013, it was used by field methods classes 
at the University of Ottawa (Teenek) and Pomona College  (Igikuria). In Winter 2014, iField is being used by field methods classes
...
We have presented iField, an open source, open-ended system which helps make it easier for field work teams to focus more on data analysis,  and less on 
 repetitive task inherent in managing an evolving data heavy project. 
iField was designed from the ground up to be both easy for new users to use, but also respect data management best practices. In the process, iField's open development and use of a unified, easy-to-learn scripting language for user interfaces, web services and data queries has empowered field linguistics labs to learn more about data automation and build internal technical knowledge, allowing teams to develop novel ways of understanding and exploring their data. %explain the subtleties of language data to database administrators.

%,helping to bridge members of the technical community with the research community.

%What doesn't iField do: it doesnt make traditional single language dictionaries, for this communities should use WeSay, it doesnt let users transcibe speech or gestures, for this use Elan or Praat. iField can be used to build collocation and illustrative dictionaries, but traditional lexicography is far far more data curation than the type of data curation which goes into field work thus teams who have promissed to create a dictionary should either allocate resources to lexicography, or reevaluate whether the langauge community would be best served with a dictionary or rather with a content site that integrates with social media and can help heritage speakers interact and share content in their minority languages. 
...
By April 2012 when iField began, NoSQL databases had matured to the extent that they were well understood, and deployed at large scale. 
Using an existing mature NoSQL database solution has greatly reduced the novel code needed to build iField, meaning we were able to focus our efforts on building usable user interfaces, and integrating our favourite field linguistics tools as modules and plugins to help automate the data entry process. 
...
\begin{exe}
  ...
  \ex   Currently human resources are very expensive while hardware is inexpensive, resulting in a growing popularity of NoSQL databases including what is under iField (CouchDB)
  \centering
...
%We believe iField's position in providing an easy to use tool which produces highly structured data makes it help advance current efforts to assist field work teams to preserve their data.
...
%As a tool for research, a software should ideally require no time for researchers to learn its functionality.  Various functions in iField are organized into widgets (e.g. data entry widget, search widget), and the user interface of each widget transparently reflects its function in order to minimize the time spent on figuring out the behaviour of the application. 
%The database software underlying iField (CouchDB) allows data schema to change, which enables researchers to customize and modify data entry fields at any point during the course of research. In addition to the default data fields (utterance, morpheme, gloss, translation) researchers can add `phonetic transcription' or `context of utterance' as data fields, for example. 
%CouchDB has the ability to deal with and record data modifications that happen concurrently by multiple users. iField's activity feed widget reports each modification to users who share the same database, making it easy for a team of researchers to keep track of their data and to work on the same database without concern of compromising or losing data. 
%Import and export functions in iField allows researchers to gather and organize data that created in different formats, and put them back into a desired format. 
...
%
%\item{\sc Data management}: iField allows number of permissions for data access and management. All data are versioned and data history is easily trackable, which enables users to recover and resolve mistakes or inconsistent annotation. Comment feeds on datums encourage users to discuss issues they discover on their data.   
%
%\item{\sc Data description}: A datum is produced according to Leipzig conventions, and iField provides four data fields (utterance, morphemes, gloss, translation) as default. Additional metadata description are configured on a corpus basis by the corpus administrator. (See also Collect below.)
%%iField offers a list of commonly used annotation fields to researchers which is generated by the popularity of fields among app users. 
%
%\item {\sc Data sharing}: Data may be shared with members of a research team through access permissions. Data may also be converted to commonly used formats (.csv, .xml, .txt, .tex) to be given directly to members who are not using iField. Users can also embed live widgets to share portions of data on project's webpages, or WordPress blogs. Users can schedule "Bots" to automatically release/publish their data to external web services or language archives.
%
...
%
%
%\item{\sc Budget}: iField is free and OpenSouce. The application can be installed for free on department servers or institutional data centres, which may require hiring professionals to maintain the database and ensure that the data is properly backed up. iField can also run on the cloud and departments can choose to host their data on a cloud hosting provider. The costs usually range between \$1 and \$10 per month for 1-100 gigabytes of data/data transfer.
%
%\end{itemize} 
%
%iField can also be used to plan and manage data collection process. Users can begin by adding a description to their corpus, as well as adding language consultants and collaborators. When they begin collecting data they are first prompted to enter a Session and encouraged to write the session goals. Users are even able to prepare hypotheses in the form of Data lists containing Datum "To be Checked" to prepare for their data elicitation session with their language consultant. In this way iField helps a researcher in their collection rationale, collection and analysis methods. When the users add fields to datum they can enter a help text which will pop-up over the datum field as users are entering if they need to know what are the conventions for that field, e.g. IPA transcription could be either phonological or phonetic, the corpus administrator can indicate the conventions using the help text. 
%
%
...
%
%The data in iField, if tagged for public view, are in principle searchable via standard search engines. Data permitted for export will be exportable to CSV, XML, text or LaTex formats. (See also \S \ref{sec:documentation} Discovery, Access \& Citation)
%
%\item{\bf Integrate}: Data from disparate sources are combined into one consistent data set. 
%
%iField can be used to integrate and organize data created in different applications. Import/Export functions enable to combine data created in different file formats (e.g. .csv from FileMaker Pro and .xml from ELAN) into .json files and export them back to the original formats. While on iField, data can be made consistent by the creation of "Bots" who crawl the corpus and automate changes.
%
...
% with an unexpected number of users, to date (January 2014) iField now has over 300 users who have created over 1000 databases.
...
%and maybe how iField data can be imported by them?
...
%It is possible to download the entire Wiktionary for a language and import or export it using iField  The dictionary module shown in Table~\ref{tab:dictionary} is used to crawl the corpus to gather citations and examples to build a Wiktionary dictionary for the corpus' language,
...
%\subsubsection{Using iField to power online language documentation sites/mobile apps}
%
%\cite{Good:2012}
...
%
%iField is a product of collaboration between field linguists,
...
%While more field database tools may surface in the coming years, we believe iField to be the only language documentation tool to take advantage of an established scalable NoSQL database solution, rather than reinventing a custom XML data store or relational database schema. 
