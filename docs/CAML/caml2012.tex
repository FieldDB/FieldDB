...
Rakshit Majithiya,
Kim Dan Nguyen,
Pablo Duboue, as well as countless other open source software developers whose code directly or indirectly helped build LingSync to what it is today and in the future. 
We would like to thank LingSync users for providing feedback, suggestions,  asking questions and sending bug reports, all of which have been instrumental in LingSync's success.
...
but it also has an installation guide for server administrators so organizations can run their own instance of LingSync. Not only is its source code 100\% open, LingSync's development is also 100\% open and driven by the research assistants of fieldwork teams who use the application.
\end{abstract}
...
these language researches often do not work on a full-time basis but rather as one of many other tasks (including, but not limited to, teaching, grading, researching and preparing publications).
...
Section \ref{sec:what} discusses LingSync's core \S \ref{sec:core} functionality as well as how LingSync empowers teams  with plugins \S  \ref{sec:plugins} to reuse and repurpose existing resources and software libraries. Section \ref{sec:use} discusses how teams are currently using LingSync, and celebrates LingSync's  growing community of over 300 users in just 1.5 years since the project was launched at CAML. Section \ref{sec:conclusion} summarizes  the paper.
...
This means that collaboration in these tools must use an ad-hoc mechanism such as DropBox, e-mail (or in an industry setting, a version control system such as SVN or Git)
...
not Mac or mobile devices, They can also require too much time and training to be practical for field linguistics labs with a high turnover rate or field methods courses: in both cases team members do not have time for data management as a full-time job \citep{Butler:2007}.
can 
...
LingSync is actually only an open ended recursive data schema (as shown in \ref{ex:intelligible}) % (Appendix \S \ref{appendix:datumjson}), 
...
to enable consistency, users see typeahead drop-down menus containing terms previously used by their team members, as shown in (\ref{ex:consistent}). At this point, users may accept existing terms from the drop-down, or add new tags and categories without needing to leave the data entry interface.
...
\begin{exe} 
\ex a. Data can be search engine discoverable. \\
b. fields can even be public yet masked 
...
Bots can even be scheduled to run periodically on the corpus (\ref{ex:bots}),
...
or even execute the changes (\ref{ex:bots}) after the team has reviewed and approved the changes.
...
For teams collaborating with Computer Science or Software Engineering departments, there are limitless plugins which port advances in Computational Linguistics  and Natural Language Processing libraries  \citep{Chen:2011}  including  negation and modality scope taggers  \citep{Rosenberg:2012}  using  GATE \citep{Cunningham:2011}, discourse extraction out of social media  \citep{Dubuc:2010}, mobile apps which take advantage of diverse NLP pipelines wrapped in web services \citep{Sateli:2013},
...
Sample web services for wrapping Bash shell scripts are provided on the OpenSourceFieldlinguistics GitHub.\footnote{https://github.com/OpenSourceFieldlinguistics} %\citep{Desilets:2010}
}
...
We have been very pleased by the positive reception so far towards LingSync by field linguists from a variety of locations and affiliations, and we gratefully acknowledge the comments from all of our early users, many of which have been used to improve it already. As of February 2014, there are over 300 people who have created user accounts with LingSync,
all of whom together have created over 1000 corpora.
Over 150 of these user accounts are field methods students, while the remainder appear to be `investigating' accounts created to try out the app.
