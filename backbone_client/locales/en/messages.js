    ...
    function() {
      var locales = {
        ...
        "locale_advanced_search_explanation" : {
          "message" : "<p>Search errs on the side of including more results, rather than missing anything. </p> <p>For example, you can type 'nay' and search will find the morphemes 'onay', 'naya' etc.. </p> <p>You can specify your search domain by separating the domain name and the search term by ':' (a colon). For example, if you type 'gloss:past', the search will look for 'past' in the gloss field and find data that contains the gloss 'past'. If you type just 'past', the search will find data that contains 'past' in any field (e.g. the gloss and the translation). </p> <p>You can restrict your search results by using regular expressions in your search term. For example, typing '\bnay' will find morphemes that begin with 'nay', typing 'nay\b' will find morphemes that end with 'nay'. Typing 'na+y' will find morphemes containing the sequences 'nay' 'naay' 'naaay' etc., '\bna+y' will find morphemes that begin with 'nay' 'naay' 'naaay' etc.. </p> <p>Search automatically creates a temporary list of data. If you enter new matching data, it will be added automatically this can be a handy way to see the data you have entered recently, as you enter data.. If you want to keep the list of data, click Save and a new DataList will be created. </p> <p>For now, search is offline, running on your device, but we would eventually like to have a more advanced search that works online, sorts results better, and could let you search for minimal pairs using features.</p>"
        },
        "locale_New_User" : {
          "message" : "New User"
        },
