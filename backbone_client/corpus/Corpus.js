define(
    [ "backbone", "comment/Comment", "comment/Comments", "corpus/CorpusMask",
        "confidentiality_encryption/Confidential", "datum/DatumField",
        "datum/DatumFields", "datum/DatumState", "datum/DatumStates",
        // "text!/_view/datalists",
        // "text!/_view/sessions",
        "data_list/DataList", "data_list/DataLists", "user/Consultants",
        "lexicon/Lexicon", "permission/Permission", "permission/Permissions",
        "datum/Session", "datum/Sessions", "user/Team", "user/User",
        "user/Users", "user/UserMask", "glosser/Glosser", "OPrime" ],
    function(Backbone, Comment, Comments, CorpusMask, Confidential, DatumField,
        DatumFields, DatumState, DatumStates,
        // forcingdataliststoloadearly,
        // forcingsessionstoloadearly,
        DataList, DataLists, Consultants, Lexicon, Permission, Permissions,
        Session, Sessions, Team, User, Users, UserMask) {
      var Corpus = Backbone.Model
          .extend(
          /** @lends Corpus.prototype */
          {
            /**
             * @class A corpus is like a git repository, it has a remote, a
             *        title a description and perhaps a readme When the user
             *        hits sync their "branch" of the corpus will be pushed to
             *        the central remote, and we will show them a "diff" of what
             *        has changed.
             * 
             ...
             * 
             * @property {Consultants} consultants Collection of consultants who
             *           contributed to the corpus
             * @property {DatumStates} datumstates Collection of datum states
             *           used to describe the state of datums in the corpus
             * @property {DatumFields} datumfields Collection of datum fields
             *           used in the corpus
             * @property {ConversationFields} conversationfields Collection of
             *           conversation-based datum fields used in the corpus
             * @property {Sessions} sessions Collection of sessions that belong
             *           to the corpus
             * @property {DataLists} datalists Collection of data lists created
             *           under the corpus
             * @property {Permissions} permissions Collection of permissions
             *           groups associated to the corpus
             * 
             * 
             ...
             *           allomorphs and glosses which are used to index datum,
             *           and also to gloss datum.
             * 
            ...
            initialize : function() {
              ...
              this.bind("change:publicCorpus", this.changeCorpusPublicPrivate,
                  this);

              // var couchConnection = this.get("couchConnection");
              // if(!couchConnection){
              // couchConnection =
              // JSON.parse(localStorage.getItem("mostRecentCouchConnection"));
            ...
            loadOrCreateCorpusByPouchName : function(pouchname,
              ...
              c
                  .fetch({
                    success : function(model, response, options) {
                      if (OPrime.debugMode)
                        OPrime.debug("Success fetching corpus' public self: ",
                            model, response, options);
                      if (!model.get("corpusid")) {
                        corpusself
                            .fillWithDefaults(sucessloadingorCreatingcallback);
                        return;
                      }
                      ...
                      corpusself
                          .fetch({
                            success : function(model) {
                              if (OPrime.debugMode)
                                OPrime.debug("Corpus fetched successfully",
                                    model);
                              $(".spinner-status").html("Loading Datalist...");
                              corpusself
                                  .makeSureCorpusHasADataList(
                                      function() {
                                        corpusself.datalists.at(0)
                                        ...
                                        $(".spinner-status").html(
                                            "Loading Elicitation Session...");
                                        corpusself
                                            .makeSureCorpusHasASession(
                                                function() {
                                                  corpusself.sessions
                                                      .at(0)
                                                      .setAsCurrentSession(
                                                  ...
                                                  // end success to create new
                                                  // data list
                                                },
                                                function() {
                                                  alert("Failed to create a session. ");
                                                });// end failure to create new
                                                    // data list
                            ...
                            error : function(model, xhr, options) {
                              $(".spinner-status")
                                  .html("Downloading Corpus...");

                              if (OPrime.debugMode)
                                OPrime.debug("Error fetching corpus  : ",
                                    model, xhr, options);
                              if (corpusself.islooping) {
                                OPrime
                                    .bug("Couldn't download this corpus to this device. There was an error replicating corpus..."
                                        + e);
                                return;
                              }
                              corpusself.islooping = true;
                              OPrime
                                  .bug("Trying to download this corpus to this device one more time..."
                                      + xhr.reason);
                              corpusself.loadOrCreateCorpusByPouchName(
                                  pouchname, sucessloadingorCreatingcallback);
                            }
                          });
                    },
                    error : function(model, xhr, options) {
                      ...
                      if (OPrime.debugMode)
                        OPrime.debug("Error fetching corpus mask : ", model,
                            xhr, options);
                      OPrime.bug("Error fetching your corpus' public view..."
                          + xhr.reason);
                      corpusself.get("publicSelf").fillWithDefaults();
                      corpusself.get("publicSelf").set("couchConnection",
                          corpusself.get("couchConnection"));
                      corpusself.get("publicSelf").set("pouchname",
                          corpusself.get("pouchname"));
                      corpusself
                          .fillWithDefaults(sucessloadingorCreatingcallback);
                    }
                  });
                ...
                this.get("publicSelf").fetch(
                    {
                      sucess : function(model, response, options) {
                        if (OPrime.debugMode)
                          OPrime.debug(
                              "Success fetching corpus' public self: ", model,
                              response, options);
                      },
                      error : function(model, xhr, options) {
                        if (OPrime.debugMode)
            ...
            fillWithDefaults : function(donefillingcallback) {
              ...
              if (!this.get("datumStates")
                  || this.get("datumStates").length == 0) {
                this.set("datumStates", new DatumStates([ new DatumState({
                  state : "Checked",
                  color : "success",
                  selected : "selected"
                }), new DatumState({
              ...
              if (!this.get("conversationFields")
                  || this.get("conversationFields").length == 0) {
                this
                    .set(
                        "conversationFields",
                        new DatumFields(
              ...
              if (!this.get("sessionFields")
                  || this.get("sessionFields").length == 0) {
                this
                    .set(
                        "sessionFields",
                        new DatumFields(
                ...
                this
                    .set(
                        "license",
                        {
                          ...
                          title : "License Terms: This license lets others remix, tweak, and build upon your work even for commercial purposes, as long as they credit you and license their new creations under the identical terms. This license is often compared to “copyleft” free and open source software licenses. All new works based on yours will carry the same license, so any derivatives will also allow commercial use. This is the license used by Wikipedia, and is recommended for materials that would benefit from incorporating content from Wikipedia and similarly licensed projects.",
            ...
            // proposed by the authors of Backbone to documents in your
            // database,
            // so that you don't have to change existing apps when you switch
            // the sync-strategy
            url : "/private_corpuses",

            loadPermissions : function(doneLoadingPermissions) {
              if (!this.get("team")) {
                // If app is completed loaded use the user, otherwise put a
                // blank user
                if (window.appView) {
                  this.set("team", window.app.get("authentication").get(
                      "userPublic"));
                  // this.get("team").id =
                  // window.app.get("authentication").get("userPublic").id;
                } else {
                  // this.set("team", new UserMask({pouchname:
                  // this.get("pouchname")}));
                }
              }
              ...
              window.app
                  .get("authentication")
                  .fetchListOfUsersGroupedByPermissions(
                      function(users) {
                        var typeaheadusers = [];
                        for ( var user in users.notonteam) {
                          if (users.notonteam[user].username) {
                            ...
                            if (OPrime.debugMode)
                              OPrime.debug("This user is invalid",
                                  users.notonteam[user]);
                          }
                        }
            ...
            /**
             * Make the model marked as Deleted, mapreduce function will ignore
             * the deleted models so that it does not show in the app, but
             * deleted model remains in the database until the admin empties the
             * trash.
             * 
            ...
            putInTrash : function() {
              OPrime
                  .bug("Sorry deleting corpora is not available right now. Too risky... ");
              return;
              /*
               * TODO contact server to delte the corpus, if the success comes
               * back, then do this
               */
            ...
            // This the function called by the add button, it adds a new comment
            // state both to the collection and the model
            insertNewComment : function(commentstring) {
              ...
              window.app
                  .addActivity({
                    ...
                    indirectobject : "on <i class='icon-cloud'></i><a href='#corpus/"
                        + this.id + "'>this corpus</a>",
                    teamOrPersonal : "team",
            ...
            newSession : function() {
              ...
              window.app.get("currentSession").saveAndInterConnectInApp(
                  function() {
                    // Clone it and send its clone to the session modal so that
                    // the users can modify the fields and then change their
                    // mind, wthout affecting the current session.
                    window.appView.sessionNewModalView.model = new Session({
                      comments : new Comments(),
                      pouchname : self.get("pouchname"),
                      sessionFields : window.app.get("currentSession").get(
                          "sessionFields").clone()
                    });
            ...
            newCorpus : function() {
              ...
              // Clone it and send its clone to the session modal so that the
              // users can modify the fields and then change their mind, wthout
              // affecting the current session.
              var attributes = JSON.parse(JSON.stringify(this.attributes));
              // Clear the current data list's backbone info and info which we
              // shouldnt clone
              ...
              // attributes.sessionFields = new
              // DatumFields(attributes.sessionFields);
              attributes.pouchname = this.get("pouchname") + "copy";
              attributes.couchConnection.pouchname = this.get("pouchname")
                  + "copy";
              ...
              attributes.team = window.app.get("authentication").get(
                  "userPublic").toJSON();
              // clear out search terms from the new corpus's datum fields
              /*
               * use default datum fields if this is going to based on teh
               * users' first practice corpus
               */
              ...
              // clear out search terms from the new corpus's conversation
              // fields
              ...
              window.appView.corpusNewModalView.model
                  .set(window.appView.corpusNewModalView.model
                      .parse(attributes));
              /*
               * use default datum fields if this is going to based on teh
               * users' first practice corpus
            ...
            // glosser: new Glosser(),//DONOT store in attributes when saving to
            // pouch (too big)
            lexicon : new Lexicon(),// DONOT store in attributes when saving to
                                    // pouch (too big)
            prepareANewPouch : function(couchConnection, callback) {
              if (!couchConnection || couchConnection == undefined) {
                console
                    .log("App.changePouch couchConnection must be supplied.");
                return;
              } else {
                console.log("prepareANewPouch setting couchConnection: ",
                    couchConnection);
              }
              // alert("TODO set/validate that the the backone couchdb
              // connection is the same as what user is asking for here");
              // $.couch.urlPrefix =
              // OPrime.getCouchUrl(window.app.get("couchConnection"),"");

              if (OPrime.isChromeApp()) {
                Backbone.couch_connector.config.base_url = window.app
                    .getCouchUrl(couchConnection, "");
             ...
             * failure to the user.
             *  - Adds the corpus to the corpus if it is in the right corpus,
             * and wasn't already there - Adds the corpus to the user if it
             * wasn't already there - Adds an activity to the logged in user
             * with diff in what the user changed.
             * 
             * @param successcallback
             * @param failurecallback
             */
            saveAndInterConnectInApp : function(successcallback,
                failurecallback) {
              if (OPrime.debugMode)
              ...
              if (!this.id) {
                /*
                 * If this is a new corpus, and we are not in it's database, ask
                 * the server to create the databse and loop until it is
                ...
                if (!this.get("couchConnection")) {
                  this.get("couchConnection").pouchname = this.get("team").get(
                      "username")
                      + "-"
                      + this.get("title").replace(/[^a-zA-Z0-9-._~ ]/g, "");
                }
                ...
                /*
                 * If its a chrome app, the user can create a new pouch offline
                 * without any problems... but they cant save to the backbone
                 * couchdb so we should still encourage them to be online when
                 * they make a new corpus.
                 */
                if (OPrime.isChromeApp()) {
                  // alert("TODO test what happens when creating a new corpus in
                  // chrome app");
                  // this.syncBeforeChangePouch = false;
                }

                /*
                 * faking the date of last datum to avoid having a old session
                 * pop up
                 */
                this.set("dateOfLastDatumModifiedToCheckForOldSession", JSON
                    .stringify(new Date()));

                delete this.get("couchConnection").corpusid;
                // make sure the corpus is in the history of the user to trigger
                // the server to create the database before we go further
                var pouches = _.pluck(window.app.get("authentication").get(
                    "userPrivate").get("corpuses"), "pouchname");
                if (pouches.indexOf(potentialpouchname) == -1) {
                  window.app.get("authentication").get("userPrivate").get(
                      "corpuses").unshift(this.get("couchConnection"));
                }
                ...
                // = "";
                // window.app.get("authentication").get("userPrivate").get("mostRecentIds").couchConnection
                // = this.get("couchConnection");
                if (this.syncBeforeChangePouch) {
                  var newCorpusToBeSaved = this;
                  window.app.showSpinner();
                  $(".spinner-status")
                      .html(
                          "Contacting the server to create a database for your corpus...");
                  window.app
                      .get("authentication")
                      .syncUserWithServer(
                          function() {
                            ...
                            /*
                             * Redirect the user to their user page, being
                             * careful to use their (new) database if they are
                             * in a couchapp (not the database they used to
                             * register/create this corpus)
                             */
                            var optionalCouchAppPath = "";
                            if (OPrime.isCouchApp()) {
                              optionalCouchAppPath = "/" + potentialpouchname
                                  + "/_design/pages/";
                            }
                            OPrime
                                .checkToSeeIfCouchAppIsReady(
                                    optionalCouchAppPath + "corpus.html",
                                    function() {
                                      // OPrime.bug("Attempting to save the new
                                      // corpus in its database.");
                                      if (OPrime.isBackboneCouchDBApp()) {
                                        try {
                                          Backbone.couch_connector.config.db_name = potentialpouchname;
                                        } catch (e) {
                                          OPrime
                                              .bug("Couldn't set the database name off of the pouchame when creating a new corpus for you, please report this.");
                                        }
                                      } else {
                                      ...
                                      newCorpusToBeSaved
                                          .prepareANewPouch(
                                              window.app.get("authentication")
                                                  .get("userPrivate").get(
                                                      "corpuses")[0],
                                              function() {
                                                // alert("Saving new corpus in
                                                // new corpus menu.");
                                                $(".spinner-status")
                                                    .html(
                                                        "Saving the corpus in your new database ...");
                                                          ...
                                                          null,
                                                          {
                                                            success : function(
                                                              ...
                                                              window.app
                                                                  ...
                                                                  .get(
                                                                      "userPrivate")
                                                                  .get(
                                                                      "mostRecentIds").corpusid = model.id;
                                                              model
                                                                  .get("couchConnection").corpusid = model.id;
                                                              window.app
                                                                  .get(
                                                                      "authentication")
                                                                  .get(
                                                                      "userPrivate")
                                                                  .get(
                                                                      "mostRecentIds").couchConnection = model
                                                                  .get("couchConnection");
                                                              window.app
                                                                  .get(
                                                                      "authentication")
                                                                    ...
                                                                    .saveAndInterConnectInApp(
                                                                        function() {
                                                                          $(
                                                                              ".spinner-status")
                                                                              .html(
                                                                                  "New Corpus saved in your user profile. Taking you to your new corpus when it is ready...");
                                                                          window
                                                                              .setTimeout(
                                                                                  function() {
                                                                                    window.location
                                                                                        .replace(optionalCouchAppPath
                                                                                            + "user.html#/corpus/"
                                                                                            + potentialpouchname
                                                                                            + "/"
                                                                                            + model.id);
                                                              ...
                                                              model
                                                                  .get(
                                                                      "publicSelf")
                                                                  .saveAndInterConnectInApp(
                                                                      sucessorfailcallbackforcorpusmask,
                                                                      sucessorfailcallbackforcorpusmask);
                                                            ...
                                                            error : function(e,
                                                              ...
                                                              $(
                                                                  ".spinner-status")
                                                                  .html(
                                                                      "New Corpus save error "
                                                                          + f.reason
                                                                          + ". The app will re-attempt to save your new corpus in 10 seconds...");
                  ...
                  if (OPrime.debugMode)
                    OPrime
                        .debug("Contacting the server to ask it to make a new database for you...");
                  return;
                }
              ...
              /*
               * For some reason the corpus is getting an extra state that no
               * one defined in it. this gets rid of it when we save.
               */
              try {
                ...
                if (OPrime.debugMode)
                  OPrime
                      .debug(
                          "Removing empty states work around failed some thing was wrong.",
                          e);
              }
              ...
              self.save(null,
                  {
                    success : function(model, response) {
                      ...
                      var differences = "#diff/oldrev/" + oldrev + "/newrev/"
                          + response._rev;
                      ...
                      if (window.appView) {
                        window.appView.toastUser("Sucessfully saved corpus: "
                            + title, "alert-success", "Saved!");
                      ...
                      var teamid = model.get("team").id; // Works if UserMask
                                                          // was saved
                      if (!teamid) {
                        teamid = model.get("team")._id; // Works if UserMask
                                                        // came from a mongodb
                                                        // id
                        if (!teamid) {
                          if (model.get("team").get("username") == window.app
                              .get("authentication").get("userPrivate").get(
                                  "username")) {
                            teamid = window.app.get("authentication").get(
                                "userPrivate").id; // Assumes the user private
                                                    // and team are the same
                                                    // user...this is dangerous
                          }
                        }
                        /**
                         * The idea of the masks in the activity is that the
                         * teams/users can make a public activity feed, which
                         * they create a special widget user for, and if the
                         * widget user asks for activities, the map reduce
                         * function returns only the masks, whcih means that the
                         * original activities are protected. so if the activity
                         * is soemthing that you might want to appear in a
                         * really public feed, then add a mask to it, and it
                         * will automatically appear. this can probably be done
                         * for all activities later. Right now its only in the
                         * syncing aspect so at least we can test the map reduce
                         * function.
                         */
                        window.app.addActivity({
                          verb : "<a href='" + differences + "'>" + verb
                              + "</a> ",
                          verbmask : verb,
                          verbicon : verbicon,
                          directobject : "<a href='#corpus/" + model.id + "'>"
                              + title + "</a>",
                          directobjectmask : "a corpus",
                          directobjecticon : "icon-cloud",
                          indirectobject : "created by <a href='#user/"
                              + teamid + "'>" + teamid + "</a>",
                          indirectobject : "created by <a href='#user/"
                              + teamid + "'>" + teamid + "</a>",
                          context : " via Offline App.",
                      ...
                      // make sure the corpus is updated in the history of the
                      // user
                      var pouches = _.pluck(window.app.get("authentication")
                          .get("userPrivate").get("corpuses"), "pouchname");
                      var oldconnection = pouches.indexOf(model
                          .get("couchConnection").pouchname);
                      if (oldconnection != -1) {
                        window.app.get("authentication").get("userPrivate")
                            .get("corpuses").splice(oldconnection, 1);
                      }
                      window.app.get("authentication").get("userPrivate").get(
                          "corpuses").unshift(model.get("couchConnection"));

                      if (newModel) {

                        self.makeSureCorpusHasADataList(function() {
                          self.makeSureCorpusHasASession(function() {
                            // save the internal models go to the user dashboard
                            // to to load the corpus into the dashboard
                            self.save(null, {
                              success : function(model, response) {
                                window.app.get("authentication")
                    ...
                    error : function(model, response, options) {
                      if (OPrime.debugMode)
                        OPrime.debug("Corpus save error", model, response,
                            options);
                      // if(response && response.reason && response.reason ==
                      // "unauthorized"){
                  ...
                  .fetch({
                    ...
                    success : function(model, response, options) {
                      if (response.length > 0) {
                        ...
                        if (OPrime.debugMode)
                          OPrime
                              .debug("You have no datalists, creating a new one...");
                        // create the first datalist for this corpus.
                        var dl = new DataList({
                          filledWithDefaults : true,
                          pouchname : self.get("pouchname")
                        }); // MUST be a new model, other wise it wont save in a
                            // new pouch.
                        dl
                            .set({
                        ...
                        if (!OPrime.isBackboneCouchDBApp()) {
                          ...
                          // dl.pouch =
                          // Backbone.sync.pouch(OPrime.isAndroidApp() ?
                          // OPrime.touchUrl + self.get("pouchname") :
                          // OPrime.pouchUrl + self.get("pouchname"));
                        }
                        dl.save(null, {
                          success : function(model, response) {
                            window.app.get("authentication").get("userPrivate")
                                .get("dataLists").unshift(model.id);
                            ...
                            if (typeof sucess == "function") {
                              ...
                              if (OPrime.debugMode)
                                OPrime
                                    .debug('DataList save success' + model.id);
                            }
                          },
                          error : function(e) {
              ...
              this.sessions
                  .fetch({
                    error : function(model, xhr, options) {
                      ...
                      if (OPrime.debugMode)
                        OPrime
                            .debug("There was an error loading your sessions.");
                      console.log(model, xhr, options);
                    ...
                    success : function(model, response, options) {
                      if (response.length > 0) {
                        ...
                        if (OPrime.debugMode)
                          OPrime
                              .debug("You have no sessions, creating a new one...");
                        var s = new Session({
                        ...
                        if (!OPrime.isBackboneCouchDBApp()) {
                          alert("TODO test this setting pouch");
                          // s.pouch = Backbone.sync.pouch(OPrime.isAndroidApp()
                          // ? OPrime.touchUrl + self.get("pouchname") :
                          // OPrime.pouchUrl + self.get("pouchname"));
                        }
                        s.save(null,
                            {
                              success : function(model, response) {
                                window.app.get("authentication").get(
                                    "userPrivate").get("sessionHistory")
                                    .unshift(model.id);
             ...
             * 
             * @returns {} an object containing valid map reduce functions TODO:
             *          add conversation search to the get_datum_fields function
             */
            ...
            createPouchView : function(view, callbackpouchview) {
              ...
              if (OPrime.isBackboneCouchDBApp()) {
                // TODO make the view in couchdb shouldnt be necessary since it
                // was created in the couchapp?
              ...
              corpusself
                  .pouch(function(err, db) {
                    var modelwithhardcodedid = {
                      ...
                      "views" : {
                      // "by_id" : {
                      // "map": "function (doc) {if (doc.dateModified)
                      // {emit(doc.dateModified, doc);}}"
                      // }
                      }
                    };
                    ...
                    if (window.validCouchViews[view].reduce) {
                      modelwithhardcodedid.views[viewparts[1]].reduce = window.validCouchViews[view].reduce
                          .toString();
                    }

                    console.log("This is what the doc will look like: ",
            ...
            setAsCurrentCorpus : function(successcallback, failurecallback) {
              ...
              window.app.get("authentication").get("userPrivate").get(
                  "mostRecentIds").corpusid = this.id;
              window.app.get("authentication").get("userPrivate").get(
                  "mostRecentIds").couchConnection = this
                  .get("couchConnection");
              ...
              if (window.appView) {
                window.appView
                    .setUpAndAssociateViewsAndModelsWithCurrentCorpus(function() {
                      if (typeof successcallback == "function") {
                        successcallback();
                      } else {
                        window.appView.toastUser(
                            "Sucessfully connected all views up to corpus: "
                                + this.id, "alert-success", "Connected!");
              ...
              if (attrs.publicCorpus) {
                if (attrs.publicCorpus != "Public") {
                  if (attrs.publicCorpus != "Private") {
                    return "Corpus must be either Public or Private"; // TODO
                                                                      // test
                                                                      // this.
                  }
                }
            ...
            set : function(key, value, options) {
              ...
              if (attributes.title) {
                attributes.titleAsUrl = attributes.title.toLowerCase().replace(
                    /[!@#$^&%*()+=-\[\]\/{}|:<>?,."'`; ]/g, "_");// this makes
                                                                  // the
                                                                  // accented
                                                                  // char
                                                                  // unnecessarily
                                                                  // unreadable:
                                                                  // encodeURIComponent(attributes.title.replace(/
                                                                  // /g,"_"));
              }
            ...
            /**
             * This function takes in a pouchname, which could be different from
             * the current corpus in case there is a master corpus with
             * more/better monolingual data.
             * 
             * @param pouchname
             * @param callback
             */
            buildMorphologicalAnalyzerFromTeamServer : function(pouchname,
                callback) {
              if (!pouchname) {
                this.get("pouchname");
              }
              ...
              Glosser.downloadPrecedenceRules(pouchname,
                  this.get("glosserURL"), callback);
            },
            /**
             * This function takes in a pouchname, which could be different from
             * the current corpus incase there is a master corpus wiht
             * more/better monolingual data.
             * 
             * @param pouchname
             * @param callback
            ...
            /**
             * This function takes in a pouchname, which could be different from
             * the current corpus incase there is a master corpus wiht more
             * representative datum example :
             * https://corpusdev.lingsync.org/lingllama-cherokee/_design/pages/_view/get_frequent_fields?group=true
             * 
             * It takes the values stored in the corpus, if set, otherwise it
             * will take the values from this corpus since the window was last
             * refreshed
             * 
            ...
            getFrequentDatumFields : function(jsonUrl, pouchname, callback) {
              if (!jsonUrl) {
                ...
                if (!pouchname) {
                  ...
                  /*
                   * if the user has overriden the frequent fields, use their
                   * preferences
                   */
                ...
                jsonUrl = couchurl
                    + "/_design/pages/_view/get_frequent_fields?group=true";
              }
              ...
              OPrime
                  .makeCORSRequest({
                    ...
                    success : function(serverResults) {
                      ...
                      try {
                        ...
                        for ( var field in serverResults.rows) {
                          ...
                          if (serverResults.rows[field].value / totalDatumCount
                              * 100 > 50) {
                            if (OPrime.debugMode)
                              OPrime
                                  .debug("Considering "
                                      + serverResults.rows[field].key
                                      + " as frequent (in more than 50% of datum) : "
                                      + serverResults.rows[field].value
                                      / totalDatumCount * 100);
                        ...
                        if (OPrime.debugMode)
                          OPrime
                              .debug(
                                  "There was a problem extracting the frequentFields, instead using defaults : ",
                                  e);
                        frequentFields = [ "judgement", "utterance",
                            "morphemes", "gloss", "translation" ];
                      }
                      if (frequentFields == []) {
                        frequentFields = [ "judgement", "utterance",
                            "morphemes", "gloss", "translation" ];
                       ...
                       */
                      var doesThisCorpusHaveSyntacticCategory = frequentFields
                          .indexOf("syntacticCategory");
                      if (doesThisCorpusHaveSyntacticCategory > -1) {
                        frequentFields.splice(
                            doesThisCorpusHaveSyntacticCategory, 1);
                      }
                      var doesThisCorpusHaveSyntacticTreeLatex = frequentFields
                          .indexOf("syntacticTreeLatex");
                      if (doesThisCorpusHaveSyntacticTreeLatex > -1) {
                        frequentFields.splice(
                            doesThisCorpusHaveSyntacticTreeLatex, 1);
                      }
                    ...
                    error : function(response) {
                      ...
                      if (typeof callback == "function") {
                        callback([ "judgement", "utterance", "morphemes",
                            "gloss", "translation" ]);
                      }
             ...
             * the current corpus incase there is a master corpus wiht more
             * representative datum example :
             * https://corpusdev.lingsync.org/lingllama-cherokee/_design/pages/_view/get_corpus_validationStati?group=true
             * 
             * It takes the values stored in the corpus, if set, otherwise it
             * will take the values from this corpus since the window was last
             * refreshed
             ...
             * @param callback
             */
            getFrequentDatumValidationStates : function(jsonUrl, pouchname,
                callback) {
              ...
              if (!jsonUrl) {
                ...
                jsonUrl = couchurl
                    + "/_design/pages/_view/get_corpus_validationStati?group=true";
              }
              ...
              OPrime
                  .makeCORSRequest({
                    ...
                    success : function(serverResults) {
                      ...
                      if (frequentStates == []) {
                        frequentStates = [ "Checked", "Deleted",
                            "ToBeCheckedWithAnna", "ToBeCheckedWithBill",
                            "ToBeCheckedWithClaude" ];
                      }
                    ...
                    error : function(response) {
                      OPrime
                          .debug("There was a problem getting the frequentDatumValidationStates, using defaults."
                              + JSON.stringify(response));
                      if (typeof callback == "function") {
                        callback([ "Checked", "Deleted", "ToBeCheckedWithAnna",
                            "ToBeCheckedWithBill", "ToBeCheckedWithClaude" ]);
                      }
                      window.getFrequentDatumValidationStatesPending = false;
            ...
            getFrequentDatumTags : function(jsonUrl, pouchname, callback) {
              ...
              if (!jsonUrl) {
                ...
                jsonUrl = couchurl
                    + "/_design/pages/_view/get_corpus_datum_tags?group=true";
              }
              ...
              OPrime
                  .makeCORSRequest({
                    ...
                    success : function(serverResults) {
                      ...
                      if (frequentTags == []) {
                        frequentTags = [ "Passive", "WH", "Indefinte",
                            "Generic", "Agent-y", "Causative", "Pro-drop",
                            "Ambigous" ];
                      }
                    ...
                    error : function(response) {
                      OPrime
                          .debug("There was a problem getting the frequentDatumTags, using defaults."
                              + JSON.stringify(response));
                      if (typeof callback == "function") {
                        callback([ "Passive", "WH", "Indefinte", "Generic",
                            "Agent-y", "Causative", "Pro-drop", "Ambigous" ]);
                      }
                      window.getFrequentDatumTagsPending = false;
            ...
            changeCorpusPublicPrivate : function() {
              // alert("TODO contact server to change the public private of the
              // corpus");
            }
          });
