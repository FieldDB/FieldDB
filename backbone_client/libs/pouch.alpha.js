...
(function() {
   ...
   * >>> Math.uuid(8, 10) // 8 character ID (base=10) "47473046" >>>
   ...
   * 
   * For original source see http://www.webtoolkit.info/ Download: 15.02.2009
  ...
  var ajax = function ajax(options, callback) {
    ...
    var onError = function(err, cb) {
      ...
      var errObj = err.responseText ? {
        status : err.status
      } : err; // this seems too clever
      try {
        errParsed = JSON.parse(err.responseText); // would prefer not to have a
                                                  // try/catch clause
    ...
    if (typeof window !== 'undefined' && window.XMLHttpRequest) {
      ...
      if (options.json) {
        ...
        options.headers['Content-Type'] = options.headers['Content-Type']
            || 'application/json';
        if (options.body && options.processData
      ...
      return request(
          ...
          function(err, response, body) {
            ...
            // CouchDB doesn't always return the right content-type for JSON
            // data, so
            // we check for ^{ and }$ (ignoring leading/trailing whitespace)
            if (!options.binary
                && (options.json || !options.processData)
                && typeof data !== 'object'
                && (/json/.test(content_type) || (/^[\s]*\{/.test(data) && /\}[\s]*$/
                    .test(data)))) {
  ...
  // history, A -> B -> C, when a document has conflicts, that is a branch in
  // the
  ...
  function doMerge(tree, path, dontExpand) {
    ...
    tree.forEach(function(branch) {
      if (branch.pos === path.pos && branch.ids[0] === path.ids[0]) {
        ...
        // want to
        // merge. If the keys match we return the longer path with the other
        // merged
  ...
  // and whether they were deleted, undeleted documents with the longest
  // revision
  // tree (most edits) win
  ...
  // TODO: check CouchDB's replication id generation, generate a unique id
  // particular
  // to this replication
  ...
  // A checkpoint lets us restart replications from when they were last
  // cancelled
  ...
  var parseDocId = function(id) {
    var ids = (typeof id === 'string') && !(/^_(design|local)\//.test(id)) ? id
        .split('/') : [ id ];
    return {
      docId : ids[0],
  ...
  if (typeof module !== 'undefined' && module.exports) {
    // use node.js's crypto library instead of the Crypto object created by
    // deps/uuid.js
    ...
    module.exports = {
      ...
      atob : function(str) {
        return decodeURIComponent(escape(new Buffer(str, 'base64')
            .toString('binary')));
      },
      btoa : function(str) {
        return new Buffer(unescape(encodeURIComponent(str)), 'binary')
            .toString('base64');
      },
      extend : extend,
      ajax : ajax,
  ...
  var PouchAdapter = function(opts, callback) {
    ...
    api.type = function() {
      return (typeof customApi._type === 'function') ? customApi._type()
          : opts.adapter;
    };
  ...
  function getHost(name) {
    ...
    // If the given name does not contain 'http:' then return a very basic
    // object
    // with no host, the current path, the given name as the database name and
    // no
    // username/password
    return {
      host : '',
  ...
  var HttpPouch = function(opts, callback) {
    ...
    var createDB = function() {
      ajax({
        ...
        if (err && err.status === 401) {
          ...
          // If there were no errros or if the only error is "Precondition
          // Failed"
          // (note: "Precondition Failed" occurs when we try to create a
          // database
          // that already exists)
    ...
    api.get = function(id, opts, callback) {
      ...
      // '/' is "_design".
      // TODO This second condition seems strange since if parts[0] ===
      // '_design'
    ...
    // Add the document given by doc (in JSON string format) to the database
    // given by host. This does not assume that doc is a new document (i.e. does
    // not
    ...
    api.allDocs = function(opts, callback) {
      ...
      if (opts.endkey) {
        params
            .push('endkey=' + encodeURIComponent(JSON.stringify(opts.endkey)));
      }
      ...
      // If keys are supplied, issue a POST request to circumvent GET query
      // string limits
    ...
    api.changes = function(opts) {
      ...
      if (Pouch.DEBUG) {
        console.log(db_url + ': Start Changes Feed: continuous='
            + opts.continuous);
      }
      ...
      // If opts.descending exists, add the descending value to the query
      // string.
  ...
  var IdbPouch = function(opts, callback) {
    ...
    api._bulkDocs = function idb_bulkDocs(req, opts, callback) {
      ...
      docInfos.forEach(function(docInfo) {
        ...
        if (!docs.length || !newEdits
            || docInfo.metadata.id !== docs[0].metadata.id) {
      ...
      function preprocessAttachments(callback) {
        ...
        var docv = 0;
        docInfos.forEach(function(docInfo) {
          var attachments = docInfo.data && docInfo.data._attachments ? Object
              .keys(docInfo.data._attachments) : [];
      ...
      function writeDoc(docInfo, callback) {
        ...
        var attachments = docInfo.data._attachments ? Object
            .keys(docInfo.data._attachments) : [];
      ...
      function updateDoc(oldDoc, docInfo) {
        docInfo.metadata.deletions = extend(docInfo.metadata.deletions,
            oldDoc.deletions);

        var merged = Pouch.merge(oldDoc.rev_tree, docInfo.metadata.rev_tree[0],
            1000);
      ...
      preprocessAttachments(function() {
        txn = idb.transaction([ DOC_STORE, BY_SEQ_STORE, ATTACH_STORE,
            META_STORE ], IDBTransaction.READ_WRITE);
    ...
    api._get = function idb_get(id, opts, callback) {
      ...
      txn.objectStore(DOC_STORE).get(id.docId).onsuccess = function(e) {
        ...
        index.get(key).onsuccess = function(e) {
          ...
          if (opts.revs) { // FIXME: if rev is given it should return ids from
                            // root to rev (don't include newer)
          ...
          if (opts.revs_info) { // FIXME: this returns revs for whole tree and
                                // should return only branch for winner
    ...
    api._getAttachment = function(id, opts, callback) {
      ...
      // This can be called while we are in a current transaction, pass the
      // context
    ...
    api._allDocs = function idb_allDocs(opts, callback) {
      ...
      var keyRange = start && end ? IDBKeyRange.bound(start, end)
          : start ? IDBKeyRange.lowerBound(start) : end ? IDBKeyRange
              .upperBound(end) : null;

      var transaction = idb
          .transaction([ DOC_STORE, BY_SEQ_STORE ], 'readonly');
    ...
    api._changes = function idb_changes(opts) {
      if (Pouch.DEBUG)
        console.log(name + ': Start Changes Feed: continuous='
            + opts.continuous);
      ...
      function fetchChanges() {
        ...
        var req = descending ? txn.objectStore(BY_SEQ_STORE).openCursor(
            IDBKeyRange.lowerBound(opts.since, true), descending) : txn
            .objectStore(BY_SEQ_STORE).openCursor(
      ...
      function onsuccess(event) {
        ...
        if (changeIdIndex !== undefined) {
          results[changeIdIndex].seq = cursor.key; // update so it has the
        ...
        index.get(cursor.value._id).onsuccess = function(event) {
          ...
          index.get(mainRev).onsuccess = function(docevent) {
            ...
            if (opts.style === 'all_docs') {
              // console.log('all docs', changeList,
              // collectLeaves(metadata.rev_tree));
  ...
  IdbPouch.valid = function idb_valid() {
    if (!document.location.host) {
      console
          .error('indexedDB cannot be used in pages served from the filesystem');
    }
    ...
    db
        .transaction(
            function(tx) {
              ...
              var seq = 'CREATE TABLE IF NOT EXISTS '
                  + BY_SEQ_STORE
    ...
    api._bulkDocs = function idb_bulkDocs(req, opts, callback) {
      ...
      function writeDoc(docInfo, callback, isUpdate) {
        ...
        function dataWritten(tx, result) {
          ...
          var sql = isUpdate ? 'UPDATE ' + DOC_STORE
              + ' SET seq=?, json=?, winningseq=(SELECT seq FROM '
              + BY_SEQ_STORE + ' WHERE rev=?) WHERE id=?' : 'INSERT INTO '
              + DOC_STORE + ' (id, seq, winningseq, json) VALUES (?, ?, ?, ?);';
          var params = isUpdate ? [ seq, JSON.stringify(docInfo.metadata),
              mainRev, docInfo.metadata.id ] : [ docInfo.metadata.id, seq, seq,
              JSON.stringify(docInfo.metadata) ];
      ...
      function saveAttachment(docInfo, digest, data, callback) {
        ...
        var sql = 'SELECT digest, json FROM ' + ATTACH_STORE
            + ' WHERE digest=?';
        tx.executeSql(sql, [ digest ], function(tx, result) {
          if (!result.rows.length) {
            ...
            sql = 'UPDATE ' + ATTACH_STORE
                + ' SET json=?, body=? WHERE digest=?';
      ...
      db.transaction(function(tx) {
        ...
        tx.executeSql(sql, [ id.docId ], function(tx, results) {
          ...
          tx.executeSql(sql, [ key ], function(tx, results) {
            ...
            if (opts.revs) {
              var path = arrayFirst(rootToLeaf(metadata.rev_tree),
                  function(arr) {
                    return arr.ids.indexOf(doc._rev.split('-')[1]) !== -1;
                  });
              path.ids.reverse();
            ...
            if (opts.revs_info) {
              doc._revs_info = metadata.rev_tree.reduce(
    ...
    api._changes = function idb_changes(opts) {
      ...
      function fetchChanges() {
        var sql = 'SELECT ' + DOC_STORE + '.id, ' + BY_SEQ_STORE + '.seq, '
            + BY_SEQ_STORE + '.json AS data, ' + DOC_STORE
            + '.json AS metadata FROM ' + BY_SEQ_STORE + ' JOIN ' + DOC_STORE
            + ' ON ' + BY_SEQ_STORE + '.seq = ' + DOC_STORE
            + '.winningseq WHERE ' + DOC_STORE + '.seq > ' + opts.since
            + ' ORDER BY ' + DOC_STORE + '.seq '
            + (descending ? 'DESC' : 'ASC');
    ...
    api._getAttachment = function(id, opts, callback) {
      ...
      function fetchAttachment(tx) {
        var sql = 'SELECT ' + BY_SEQ_STORE + '.json AS data FROM ' + DOC_STORE
            + ' JOIN ' + BY_SEQ_STORE + ' ON ' + BY_SEQ_STORE + '.seq = '
            + DOC_STORE + '.seq WHERE ' + DOC_STORE + '.id = "' + id.docId
            + '"';
        tx.executeSql(sql, [], function(tx, result) {
  ...
  var MapReduce = function(db) {
    ...
    function viewQuery(fun, options) {
      ...
      var emit = function(key, val) {
        ...
        if (options.startkey && Pouch.collate(key, options.startkey) < 0)
          return;
        if (options.endkey && Pouch.collate(key, options.endkey) > 0)
          return;
        if (options.key && Pouch.collate(key, options.key) !== 0)
          return;
        num_started++;
