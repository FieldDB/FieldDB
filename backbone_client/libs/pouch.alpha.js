(function() {
  ...
  (function() {
    ...
    // To ensure we dont grow the revision tree infinitely, we stem old
    // revisions
    ...
    function doMerge(tree, path, dontExpand) {
      ...
      tree.forEach(function(branch) {
        if (branch.pos === path.pos && branch.ids[0] === path.ids[0]) {
          // Paths start at the same position and have the same root, so they
          // need
          // merged
  ...
  }).call(this);// Pretty dumb name for a function, just wraps callback calls so
                // we dont
  ...
  var HttpPouch = function(opts, callback) {
    ...
    api.query = function(fun, opts, callback) {
      ...
      ajax({
        auth : host.auth,
        type : 'GET',
        url : genUrl(host, '_design/' + parts[0] + '/_view/' + parts[1]
            + params),
      }, callback);
    };
    ...
    api.changes = function(opts, callback) {
      ...
      console.info(db_url + ': Start Changes Feed: continuous='
          + opts.continuous);
  ...
  var IdbPouch = function(opts, callback) {
    ...
    api.bulkDocs = function idb_bulkDocs(req, opts, callback) {
      ...
      var keyRange = IDBKeyRange.bound(docs[0].metadata.id,
          docs[docs.length - 1].metadata.id, false, false);

      txn.objectStore(DOC_STORE).openCursor(keyRange, IDBCursor.NEXT).onsuccess = readDoc;
      ...
      function txnComplete(event) {
        ...
        results.forEach(function(result) {
          ...
          var rev = winningRev(metadata.rev_tree[0].pos,
      ...
      function saveAttachment(digest, data) {
        txn.objectStore(ATTACH_STORE).put({
          digest : digest,
          body : data
        });
      }
    ...
    api.get = function idb_get(id, opts, callback) {
      ...
      txn.objectStore(DOC_STORE).get(id).onsuccess = function(e) {
        ...
        txn.objectStore(BY_SEQ_STORE).get(metadata.seq).onsuccess = function(e) {
          ...
          doc._id = metadata.id;
          doc._rev = winningRev(metadata.rev_tree[0].pos,
              metadata.rev_tree[0].ids);
          if (opts.revs) {
    ...
    api.allDocs = function idb_allDocs(opts, callback) {
      ...
      var keyRange = start && end ? IDBKeyRange.bound(start, end, false, false)
          : start ? IDBKeyRange.lowerBound(start, true) : end ? IDBKeyRange
              .upperBound(end) : false;
    ...
    api.changes = function idb_changes(opts, callback) {
      ...
      console
          .info(name + ': Start Changes Feed: continuous=' + opts.continuous);
      ...
      function fetchChanges() {
        ...
        var req = descending ? txn.objectStore(BY_SEQ_STORE).openCursor(
            IDBKeyRange.lowerBound(opts.seq, true), descending) : txn
            .objectStore(BY_SEQ_STORE).openCursor(
                IDBKeyRange.lowerBound(opts.seq, true));
      ...
      function onsuccess(event) {
        if (!event.target.result) {
          ...
          return call(opts.complete, null, {
            results : results
          });
        }
        var cursor = event.target.result;
