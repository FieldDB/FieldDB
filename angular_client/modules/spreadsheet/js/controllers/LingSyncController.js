		...
		function(angular) {
			var LingSyncController = function($scope, $rootScope, $resource,
				...
				if (LingSyncPreferences == undefined) {
					...
					console
							.log("Loading LingSyncPreferences from localStorage.");
					...
					LingSyncData
							...
							.then(
									function(newRecord) {
										...
										LingSyncData.saveNew(DB, newRecord)
					...
					LingSyncData
							...
							.then(
									function(editedRecord) {
										...
										// in scope
										LingSyncData.saveEditedRecord(DB,
												docID, editedRecord).then(
												function() {
													loadData();
												});
									});
				...
				$scope.deleteRecord = function(id, rev) {
					...
					if (r == true) {
						LingSyncData.removeRecord(DB, id, rev).then(
								function(response) {
									loadData();
								});
					}
				...
				$scope.numberOfResultPages = function(numberOfRecords) {
					var numberOfPages = Math.ceil(numberOfRecords
							/ $rootScope.resultSize);
					/*
					 * Return to first page of results if filtering makes
					 * current page empty.
					 */
