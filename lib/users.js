...
findOrCreateByUsernameData = function(userData, promise) {
  console.log("Creating or checking a pasword user"+util.inspect(userData));
  users.view('docs/username', {key: userData.username}, function(err, docs) {
    if (err) {
      console.log("Error using users/_design/docs/_view/username:");
      console.log('error in findOrCreateByUsernameData'+util.inspect(err));
      promise.fail(err);
      return err;
    }
