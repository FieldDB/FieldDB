  ...
  .exists(function(err, exists) {
    if (err) {
      ...
      .create(function(err, res) {
        ...
        var blockNonContribAdminWrites = function(new_doc, old_doc,userCtx) {
          if ((userCtx.roles.indexOf(+dbConnection.pouchname + "_"+ contributor) == -1)
              && (userCtx.roles.indexOf(+dbConnection.pouchname + "_"+ admin) == -1)) {
            throw ({
              forbidden : "Not Authorized, you are not a "
                + contributor + " on " + dbConnection.pouchname
                + ", you will have to ask " + user.username
                + " to add you as a " + contributor
...
//                + util.inspect(doc));
//          } else {
//            console.log(new Date()+ " pages/by_date created");
//          }
//        });
...
//                + util.inspect(doc));
//          } else {
//            console.log(new Date()+ " pages/get_datum_fields created");
//          }
//        });
        ...
        if (err) {
          ...
          db
              .create(function(err, res) {
                 ...
                 */
                var blockNonContribAdminWrites = function(new_doc, old_doc,userCtx) {
                  if ((userCtx.roles.indexOf(dbConnection.pouchname.replace("-activity_feed", "") + "_" + contributor) == -1)
                      && (userCtx.roles.indexOf(dbConnection.pouchname.replace("-activity_feed", "")+ "_" + admin) == -1)) {
                    throw ({
                      forbidden : "Not Authorized, you are not a "
                        + contributor
                        + " on "+ dbConnection.pouchname.replace("-activity_feed", "")
                        + ", you will have to ask " + user.username
                        + " to add you as a " + contributor
...
//                    
//                  } else {
//                    console.log(new Date()+ " pages by date created");
//                  }
//                });
    ...
    if (err) {
      ...
      db.create(function(err, res) {
        ...
        var blockNonContribAdminWrites = function(new_doc, old_doc, userCtx) {   
          if( (userCtx.roles.indexOf(dbConnection.pouchname+ "_"+ contributor) == -1 ) 
            && (userCtx.roles.indexOf( dbConnection.pouchname+ "_"+ admin) == -1 ) ) {     
          throw({forbidden: "Not Authorized, you are not a "+ contributor
            + " on "+ dbConnection.pouchname
            + ", you will have to ask " + user.username
            + " to add you as a "+ contributor
            + ". You currently have these roles: "+userCtx.roles});   
          } 
