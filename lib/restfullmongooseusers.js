...
var createDbaddUser = function(dbConnection, user, successcallback, errorcallback){
  ...
  db.exists(function (err, exists) {
    if (err) {
      ...
      console.log('Database/corpus ' + dbConnection.corpusname
      ...
      .create(function(err, res) {
         ...
         * still use iField to perform a fine grained control by
         * creating a special ifieldpublicuser which is essentially the
         * checkbox that the user can check to make the corpus private,
         * and adding all ifieldusers to a role ifieldusers which can
         * let the user make the corpus private to the world, but
         * viewable by ifieldusers (to let only signed in users comment
         * on their data etc)
         * 
         * By default: -signed in ifieldusers can read other user's
         * corpora until the user takes that role off -public user (ie
         * the general public) can see the user's corpora through
         * ifield, but not directly the couch database. This is how the
         * public checkbox is implemented in ifield.
         * 
        ...
        var securityParamsforNewDB = {
            ...
            "members" : {
              ...
              "roles" : [
                         '"' + dbConnection.corpusname + "_" + collaborator
                         + '"',
         ...
         * add them to the ifielduser role so that others can let them
         * see their corpora if they decide to let logged in ifieldusers
         * see their corpus.
         * 
         * references:
...
+"                        // words in it\n"
+"                        if (doc.datumFields[key].value.indexOf('*') >= 0) {\n"
+"                          return;\n"
+"                        }\n"
+"                        break;\n"
+"                      }\n"
...
+"                        // Trim whitespace\n"
+"                        var utterance = doc.datumFields[key].value.trim();\n"
          ...
          +"if (doc.audioVideo) {\n"
            ...
            +"for (var key in doc.datumFields) {\n"
              ...
              +"if (doc.datumFields[key].label == 'morphemes') {\n"
                +"// Trim whitespace\n"
                +"var morphemesline = doc.datumFields[key].value.trim();\n"
                +"// Tokenize the morphemes\n"
                +"var morphemes = morphemesline.toLowerCase().replace(/#!.,\\//g,'').split(/[ ]+/);\n"
                +"for (var morphemegroup in morphemes) {\n"
                  +"// If the token it not null or the empty string\n"
                  +"if (morphemes[morphemegroup]) {\n"
                    +"// Replace (*_) with ''\n"
                    +"var feederWord = morphemes[morphemegroup].replace(/\\(\\*[^)]*\\)/g, '');//DONT replace ? it is used to indicate uncertainty with the data\n"
                    +"// Replace *(_) with _\n"
                    +"feederWord = feederWord.replace(/\\*\\(([^)]*)\\)/, '$1');\n"
                    +"// Remove all parentheses and *\n"
                    +"var fullWord = feederWord.replace(/[(*)]/g, '');\n"
              ...
              +"if (doc.datumFields[key].label == 'gloss') {\n"
                +"// Trim whitespace\n"
                +"var gloss = doc.datumFields[key].value.trim();\n"
                +"// Tokenize the gloss\n"
                +"var glosses = gloss.toLowerCase().replace(/#!.,\\//g,'').split(/[ ]+/);//DONT replace ? it is used to indicate uncertainty with the data\n"
