...
var getCorpusMask = function(dbname, next) {
  ...
  if (!dbname || validateIdentifier.identifier.length < 3 || !validateIdentifier.equivalent()) {
    if (validateIdentifier.changes.length === 2 && validateIdentifier.identifier.length == validateIdentifier.original.length) {
...
var getCorpusMaskFromTitleAsUrl = function(userMask, titleAsUrl, next) {
  ...
  if (!userMask.corpora || !userMask.corpora.length) {
    err = new Error("Couldn't find any corpora for " + userMask.username + ", trying to construct the dbname");
    console.log(err);
    var dbname = userMask.username + "-" + titleAsUrl;
    return getCorpusMask(dbname, next);
    // err.status = 404;
    // deferred.reject(err);
    // next(err);
    // return deferred.promise;
