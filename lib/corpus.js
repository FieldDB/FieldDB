...
var getCorpusMaskFromTitleAsUrl = function(userMask, titleAsUrl, next) {
  ...
  matchingCorpusConnections.map(function(connection) {
    ...
    if (connection.dbname && (connection.titleAsUrl.indexOf(titleAsUrl) > -1 || connection.dbname.indexOf(titleAsUrl) > -1)) {
      if (!bestMatch) {
  ...
  // console.log(new Date() + " best match " + bestMatch.dbname);
