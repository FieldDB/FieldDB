...
exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      usersdb.insert(user.username, user, function(error, resultuser) {
        var returndata = {};
        if (error) {
...
function findById(id, fn) {
  ...
  usersdb.get(id, function(error, result) {
    var returndata = {};
    returndata.errors = error;
    if (error) {
      console.log(new Date() + ' Error looking up the user ' + id
          + ' please report this bug. ' + util.inspect(error));
      return fn([ 'Error looking up the user ' + id
          + ' please report this bug. ' ], returndata);
    } else {
      if (result) {
        returndata.user = result;
        console.log(new Date() + ' User found: ' + result);
        return fn(null, returndata);
      } else {
        console.log(new Date() + ' No User found: ' + id);
        return fn([ 'User ' + id + ' does not exist' ], returndata);
      }
    }
  });
}
