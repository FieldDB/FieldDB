...
if(emailWhenServerStarts != ""){
  ...
  console.log("Didnt email the devs: The LingSync server has restarted. (It might have crashed)");
}
    ...
    verifyPassword(password, user, function(err, passwordCorrect) {
      ...
      if(req.body.syncDetails == "true"){
        ...
        if(user.corpuses != req.body.syncUserDetails.corpuses){
          console.log(new Date()+ " It looks like the user has created some new local corpora. Attempting to make new corpus on the team server so the user can sync.");
          var userforcouchdb = {
              username : req.body.username,
              password : req.body.password,
          ...
          createNewCorpusesIfDontExist(user, userforcouchdb, req.body.syncUserDetails.corpuses);
 ...
 * password.
 * 
 * Then adds the role to the user if they are themselves
 */
module.exports.addRoleToUser = function(req, done) {
  var dbConn = req.body.couchConnection;
  if(!dbConn){
    return done({error: "Client didn't define the database connection."}, null, {message: "Problem adding role to user"});
  }
  var rolesSimpleStrings = req.body.roles;
  if(!rolesSimpleStrings){
    return done({error: "Client didnt define the roles to add."}, null, {message: "Problem adding role to user"});
  }
  var roles = [];
  ...
  corpus_management.addRoleToUser(
      dbConn,
