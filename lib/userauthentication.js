...
//      id : users.length + 1,
    ...
    var couchconnection = new cradle.Connection(node_config.usersDbConnection.protocol
        + node_config.usersDbConnection.domain, node_config.usersDbConnection.port, {
      auth : {
        username : couch_keys.username,
        password : couch_keys.password
      }
    });
    var usersdb = couchconnection.database(node_config.usersDbConnection.dbname);

    // Put the user in the database and callback
    usersdb.save(util.inspect(user), function(error, resultuser) {
      if (error) {
