...
function addCorpusToUser(error, username, newcorpusConnection, rolesresult, done) {
  if(error){
    return done(error, null, {message: "There was a problem adding user to this corpus."});
  }
  findByUsername(
      username,
      function(err, user) {
        ...
        var indexOfNewCorpusInExistingCorpuses = user.corpuses.indexOf(newcorpusConnection);
        ...
        saveUpdateUserToDatabase(user, function(error, user, info){
          if(error){
            return doneAddingCorpusToUser(error, rolesresult, {message: "Added role(s) to "+username+" but we weren't able to add this corpus to their account. This is most likely a bug, please report it."});
          }
          ...
          if (user.email && user.email.length > 5
            ...
            console.log(new Date() + " Didnt email welcome to corpus to new user"
                + user.username + " why: emailpresent: " + user.email
                + ", valid user email: " + (user.email.length > 5)
                + ", mailconfig: " + (mail_config.mailConnection.auth.user != ""));
          }
