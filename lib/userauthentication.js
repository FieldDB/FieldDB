...
module.exports.registerNewUser = function(localOrNot, req, done) {
  ...
  findByUsername(req.body.username, function(err, user) {
    if (user) {
      ...
      saveUpdateUserToDatabase(user, done);
    ...
    verifyPassword(password, user, function(err, passwordCorrect) {
      ...
      if (!passwordCorrect) {
        ...
        if(timeToSendAnEmailEveryXattempts){
          ...
          /*
           * This emails the user, if the user has an email, if the email is 'valid' and if the mail_config has a valid user. 
      ...
      console
          .log(new Date() + ' User found, and password verified ' + username);
...
function findById(id, fn) {
  ...
  usersdb.get(id, function(error, result) {
    if (error) {
      ...
      if (result) {
        console.log(new Date() + ' User found: \n' + util.inspect(result));
