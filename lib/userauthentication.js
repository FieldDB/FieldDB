...
exports.registerNewUser = function(localOrNot, req, done) {
...
//          console.log(new Date()+' Registering new user: '+ util.inspect(user));
          ...
          usersdb.insert(user, function(error, resultuser) {
            ...
            if (error) {
              ...
              console.log("No error creating a user: " + util.inspect(resultuser));
...
/*
 * TODO turn this into a couch function, or update the users array with a couch
 * view.
 */
function findById(id, fn) {
  ...
  usersdb.get(id, function(error, result) {
    if (error) {
      return fn(new Error('User ' + id + ' does not exist: '+error));
    } else {
      if (result) {
...
exports.findByUsername = function(username, done) {
  var usersdb = nano.db.use('zfielddbuserscouch');
  usersdb.view('findUsers', 'IdOrUsername', function(error, result) {
    if(error){
      console.log(new Date()+' Error connecting the user database.'+error);
      return done(new Error('Error connecting the user database.'));
    }
    if (!result) {
      console.log(new Date()+' There are no users.');
      return done(new Error('There are no users'));
    }
    console.log(new Date()+' There are '+result.length+' users.');
    var found = false;
    for ( var j = 0; j < result.rows.length; j++) {
      if (result.rows[j].username == username) {
        found  = true;
        findById(row.id, function(err, user) {
          return done(err, user);
        });
      }else{
        if(!found && j+1 == result.rows.length){
          console.log(new Date()+' There was no such user: '+username);
